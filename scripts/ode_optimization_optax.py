...
def run_script(n_qubits,n_layers,key,output_json=None):
    #### Solver Config ####
    INPUT_DIM = 1
    N_QUBITS = n_qubits
    N_LAYERS = n_layers
    ...
    ## Optimization
    linesearch = optax.scale_by_zoom_linesearch(max_linesearch_steps=25,
                                                max_learning_rate=1.0,
                                                slope_rtol=1e-4,
                                                curv_rtol=0.9,
                                                initial_guess_strategy="one",
                                                verbose=False)
    opt = optax.lbfgs(memory_size=100,
    ...
    def run_opt(init_vars,max_iter,tol):
        def step(carry):
            ...
            updates,state = opt.update(grad,state,vars,value=value,
        ...
        fin_vars,fin_state,fin_value = jax.lax.while_loop(continuing_criterion,
                                                          step, init_carry)
        return fin_vars,fin_state,fin_value
    ...
    if output_json:
        metrics = { "loss": float(value),
                    "time": float(compile_and_run_time) }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_json", type=str, default=None,
                        help="Path to save execution metrics as JSON")
    args = parser.parse_args()
    run_script(args.n_qubits,args.n_layers,args.key,args.output_json)
